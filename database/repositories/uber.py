...
class UberRepository:
    ...
    async def get_uber_progress(self, user_id: str, server_id: str) -> Dict[str, int]:
        ...
        cursor = await self.connection.execute(
            "SELECT celestial_sigils, celestial_engrams, celestial_blueprint_unlocked FROM uber_progress WHERE user_id = ? AND server_id = ?",
            (user_id, server_id)
        )
    ...
    async def set_blueprint_unlocked(self, user_id: str, server_id: str, unlocked: bool) -> None:
        """Sets the blueprint unlocked flag (1 for True, 0 for False)."""
