...
class MawRepository:
    ...
    async def get_record(self, user_id: str, cycle_id: int) -> Optional[dict]:
        cursor = await self.connection.execute(
            """SELECT signup_timestamp, last_fight_ts, damage_dealt,
                      rewards_collected,
                      COALESCE(fights_this_cycle, 0) AS fights_this_cycle
    ...
    async def get_cycle_total_damage(self, cycle_id: int) -> int:
        ...
        cursor = await self.connection.execute(
            "SELECT COALESCE(SUM(damage_dealt), 0) AS total FROM maw_participants WHERE cycle_id = ?",
            (cycle_id,),
        )
        row = await cursor.fetchone()
    ...
    async def count_participants(self, cycle_id: int) -> int:
        cursor = await self.connection.execute(
            "SELECT COUNT(*) AS cnt FROM maw_participants WHERE cycle_id = ?",
            (cycle_id,),
        )
        row = await cursor.fetchone()
