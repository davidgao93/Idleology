...
# Core Imports
from core.models import Glove
from core.items.factory import create_glove
...
class Gloves(commands.Cog, name="gloves"):
    ...
    @app_commands.command(name="gloves", description="View your character's gloves and modify them.")
    async def view_gloves(self, interaction: Interaction) -> None:
        ...
        while True:
            # A. Fetch Data
            gloves = await self._fetch_and_parse_gloves(user_id)
            
            # Handle Empty Inventory
            if not gloves:
                if message:
                    await message.edit(content="Your glove pouch is empty.", embed=None, view=None)
                else:
            ...
            # C. Pagination Math
            total_pages = (len(gloves) + items_per_page - 1) // items_per_page
            current_page = min(current_page, max(0, total_pages - 1))
            
            # D. Slice Items
            start_idx = current_page * items_per_page
            page_items = gloves[start_idx:start_idx + items_per_page]
            ...
            try:
                ...
                if cid == "close": 
                    ...
                    selected_glove = page_items[idx]
                    await self._handle_item_actions(interaction, message, selected_glove)
    ...
    async def _handle_item_actions(self, interaction: Interaction, message: Message, glove: Glove):
        ...
        while True:
            ...
            # Glove Enchanting (Potential)
            # Legacy logic capped glove passives at level 5
            can_improve = glove.potential_remaining > 0 and glove.passive_lvl < 5
            if can_improve:
            ...
            try:
                ...
                if cid == "back": return
                    ...
                    if is_equipped:
                        ...
                        continue
                    if await self._send_item_flow(message, act.user, interaction, glove):
                        return
    ...
    async def _improve_potential_flow(self, message: Message, user, glove: Glove):
        """Handles the Enchanting/Potential UI and Logic."""
        uid, gid = str(user.id), str(message.guild.id)
        
        while True:
            cost = EquipmentMechanics.calculate_ap_cost(glove.passive_lvl)
            player_gold = (await self.bot.database.users.get(uid, gid))[6]
            
            # Success Rate
            success_rate = max(75 - (glove.passive_lvl * 5), 30)
            
            title_keyword = "Enchant" if glove.passive == "none" else "Enhance"
            
            embed = discord.Embed(
                title=f"{title_keyword} Attempt", 
                description=(f"Attempts left: **{glove.potential_remaining}**\n"
                            f"Cost: **{cost:,} GP**\n"
    ...
    async def _send_item_flow(self, message: Message, user, interaction: Interaction, glove: Glove) -> bool:
        """Handles sending item logic. Returns True if sent."""
        uid, gid = str(user.id), str(message.guild.id)
        
        embed = discord.Embed(title=f"Send {glove.name}", description="Please mention the user (@username) to send this item to.", color=discord.Color.blue())
        await message.edit(embed=embed, view=None)
        ...
        try:
            ...
            if (glove.level - rec_level > 15) and rec_level < 100:
