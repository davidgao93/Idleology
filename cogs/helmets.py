...
from core.items.factory import create_helmet
from core.items.equipment_mechanics import EquipmentMechanics
from core.ui.inventory import InventoryUI

class Helmets(commands.Cog, name="helmets"):
    def __init__(self, bot) -> None:
        self.bot = bot

    async def _fetch_and_parse_helmets(self, user_id: str) -> list[Helmet]:
        """Helper to fetch raw DB data and convert to Objects."""
        raw_items = await self.bot.database.equipment.get_all(user_id, 'helmet')
        if not raw_items: 
            return []
        return [create_helmet(item) for item in raw_items]

    @app_commands.command(name="helmets", description="View your character's helmets and modify them.")
    async def view_helmets(self, interaction: Interaction) -> None:
        ...
        while True:
            # A. Fetch Data
            helmets = await self._fetch_and_parse_helmets(user_id)
            
            # Handle Empty Inventory
            if not helmets:
                if message:
                    await message.edit(content="Your helmet pouch is empty.", embed=None, view=None)
                else:
                    await interaction.response.send_message("You search your gear for helmets, but find none.")
                break

            # B. Sort: Equipped first, then by Level
            equipped_raw = await self.bot.database.equipment.get_equipped(user_id, 'helmet')
            equipped_id = equipped_raw[0] if equipped_raw else None
            
            helmets.sort(key=lambda h: (h.item_id == equipped_id, h.level), reverse=True)
            
            # C. Pagination Math
            total_pages = (len(helmets) + items_per_page - 1) // items_per_page
            current_page = min(current_page, max(0, total_pages - 1))
            
            # D. Slice Items
            start_idx = current_page * items_per_page
            page_items = helmets[start_idx:start_idx + items_per_page]
            ...
            try:
                ...
                if cid == "close": 
                    ...
                    selected_helmet = page_items[idx]
                    await self._handle_item_actions(interaction, message, selected_helmet)
    ...
    async def _handle_item_actions(self, interaction: Interaction, message: Message, helmet: Helmet):
        ...
        while True:
            ...
            # Check equipped
            equipped_raw = await self.bot.database.equipment.get_equipped(user_id, 'helmet')
            is_equipped = equipped_raw and equipped_raw[0] == helmet.item_id

            embed = InventoryUI.get_item_details_embed(helmet, is_equipped)
            
            # Add Action Guide
            guide = ["- Equip/Unequip", "- Discard", "- Back"]
            
            # Helmet Enchanting (Potential)
            # Max passive level is 5
            can_improve = helmet.potential_remaining > 0 and helmet.passive_lvl < 5
            if can_improve:
                guide.insert(1, "- Enchant (Gold)")
            ...
            if can_improve:
                view.add_item(Button(label="Enchant", style=ButtonStyle.success, custom_id="improve"))
            ...
            try:
                ...
                if cid == "back": return
                    ...
                    if is_equipped: await self.bot.database.equipment.unequip(user_id, 'helmet')
                    else: await self.bot.database.equipment.equip(user_id, helmet.item_id, 'helmet')
                elif cid == "discard":
                    if await self._confirm_action(message, act.user, f"Discard **{helmet.name}**? Irreversible."):
                        await self.bot.database.equipment.discard(helmet.item_id, 'helmet')
                        return
                elif cid == "improve":
                    await self._improve_potential_flow(message, act.user, helmet)
                elif cid == "send":
                    if is_equipped:
                        ...
                        continue
                    if await self._send_item_flow(message, act.user, interaction, helmet):
                        return
    ...
    async def _improve_potential_flow(self, message: Message, user, helmet: Helmet):
        ...
        while True:
            ...
            # Success Rate
            success_rate = max(75 - (helmet.passive_lvl * 5), 30)
            
            title_keyword = "Enchant" if helmet.passive == "none" else "Enhance"
            
            embed = discord.Embed(
                title=f"{title_keyword} Attempt", 
                description=(f"Attempts left: **{helmet.potential_remaining}**\n"
                            f"Cost: **{cost:,} GP**\n"
            ...
            try:
                ...
                # Roll logic - Helmets do not use runes currently (False)
                success = EquipmentMechanics.roll_potential_outcome(helmet.passive_lvl, use_rune=False)
                
                result_embed = discord.Embed(title="Enchant Result", color=discord.Color.gold())
                
                if success:
                    if helmet.passive == "none":
                        new_passive = EquipmentMechanics.get_new_passive('helmet')
                        await self.bot.database.equipment.update_passive(helmet.item_id, 'helmet', new_passive)
                        await self.bot.database.equipment.update_counter(helmet.item_id, 'helmet', 'passive_lvl', 1)
                        result_embed.description = f"🎉 Success! Unlocked **{new_passive.replace('-', ' ').title()}**!"
                    else:
                        new_lvl = helmet.passive_lvl + 1
                        await self.bot.database.equipment.update_counter(helmet.item_id, 'helmet', 'passive_lvl', new_lvl)
                ...
                await self.bot.database.equipment.update_counter(helmet.item_id, 'helmet', 'potential_remaining', helmet.potential_remaining - 1)
    ...
    async def _send_item_flow(self, message: Message, user, interaction: Interaction, helmet: Helmet) -> bool:
        """Handles sending item logic. Returns True if sent."""
        uid, gid = str(user.id), str(message.guild.id)
        
        embed = discord.Embed(title=f"Send {helmet.name}", description="Please mention the user (@username) to send this item to.", color=discord.Color.blue())
        await message.edit(embed=embed, view=None)
        ...
        try:
            ...
            if (helmet.level - rec_level > 15) and rec_level < 100:
                embed.description = "Item level gap is too high (>15)."
                await message.edit(embed=embed)
                await asyncio.sleep(2)
                return False
            
            rec_count = await self.bot.database.equipment.get_count(str(receiver.id), 'helmet')
            if rec_count >= 58:
            ...
            if await self._confirm_action(message, user, f"Send **{helmet.name}** to {receiver.mention}?"):
                await self.bot.database.equipment.transfer(helmet.item_id, str(receiver.id), 'helmet')
                embed.title = "Sent!"
