...
class Duels(commands.Cog, name="duels"):
    ...
    @app_commands.command(
        name="duel", description="Challenge another player to a gold duel."
    )
    @app_commands.describe(amount="Amount of gold to wager")
    async def duel(
        self, interaction: Interaction, opponent: discord.Member, amount: int
    ):
        ...
        if opponent.bot or challenger_id == target_id:
            return await interaction.response.send_message(
                "You cannot duel bots or yourself.", ephemeral=True
            )

        if amount <= 0:
            return await interaction.response.send_message(
                "Wager must be positive.", ephemeral=True
            )

        # 2. State & Registration Checks
        # Check Challenger
        c_data = await self.bot.database.users.get(challenger_id, server_id)
        if not await self.bot.check_user_registered(interaction, c_data):
            return
        if not await self.bot.check_is_active(interaction, challenger_id):
            return
        ...
        if not t_data:
            return await interaction.response.send_message(
                f"{opponent.display_name} is not registered.", ephemeral=True
            )
