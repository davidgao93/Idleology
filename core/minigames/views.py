...
class CasinoMenuView(View):
    ...
    async def _verify_funds(self, interaction: Interaction) -> bool:
        ...
        if user_data[6] < self.bet_amount:
            await interaction.response.edit_message(
                content="Insufficient funds!", embed=None, view=None
            )
            return False
        return True
    ...
    async def blackjack_btn(self, interaction: Interaction, button: Button):
        if not await self._verify_funds(interaction):
            return
        await interaction.response.edit_message(
            content=f"Starting **Blackjack** ({self.bet_amount:,})...",
            embed=None,
            view=None,
        )
    ...
    @discord.ui.button(
        label="Horse Racing", emoji="🐎", style=ButtonStyle.secondary, row=1
    )
