...
class VoidforgeView(BaseUpgradeView):
    ...
    async def select_callback(self, interaction: Interaction):
        ...
        self.selected_target = next((w for w in self.candidates if w.item_id == target_id), None)
        
        if not self.selected_target: return

        self.clear_items()
        
        confirm_btn = Button(label="CONFIRM SACRIFICE", style=ButtonStyle.danger, emoji="⚠️")
    ...
    async def execute_voidforge(self, interaction: Interaction):
        ...
        user_gold = await self.bot.database.users.get_gold(self.user_id)
        if user_gold < self.gold_cost:
            return await interaction.response.send_message("You no longer have enough gold!", ephemeral=True)
        ...
        inventory_view.total_pages = max(1, (len(inventory_view.items) + inventory_view.items_per_page - 1) // inventory_view.items_per_page)
