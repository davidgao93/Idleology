...
BUILDING_INFO = {
    ...
    "market": "Passive: Produces **5,000 Gold/hr per 100 Workers** at T1 passively, scaling with tier (up to 25,000/hr at T5). Accrues over real time only — collect it from the dashboard.",
    "barracks": "Passive: Grants **+1% of Flat Attack and Defence as Bonus Attack and Defence per 100 Workers** (applied globally during combat). Scales with tier — higher tiers raise the worker cap, increasing the maximum bonus.",
...
_BM_OFFERABLE_NAMES: list[tuple[str, str]] = [
    # (resource_key, plain_label)
    ("timber", "Timber"),
    ("stone", "Stone"),
    ("iron_ore", "Iron Ore"),
    ("coal_ore", "Coal"),
...
BM_OFFERABLE_RESOURCES: list[tuple[str, str]] = [
    (key, f"{RESOURCE_EMOJI[key]} {name}") for key, name in _BM_OFFERABLE_NAMES
]
