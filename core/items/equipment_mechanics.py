...
class EquipmentMechanics:
    ...
    @staticmethod
    def roll_temper_outcome(
        armor: Armor, bonus_chance: int = 0
    ) -> Tuple[bool, str, int]:
        """
        ...
        if armor.pdr > 0:
            ...
            expected = 5
            amount = max(1, int(random.uniform(expected * 0.7, expected * 1.3)))
        elif armor.fdr > 0:
            stat = "fdr"
    ...
    @staticmethod
    def get_new_passive(
        item_type: Literal["accessory", "glove", "boot", "helmet"],
    ) -> str:
